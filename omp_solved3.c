...
/*code problem:
 * 1. In the critical region, only one thread will be excuted at the same time, but omp barrier is used after it. Therefore, these two thread will be waiting for each other which leads to deadlock.
 * I fixed the bug by commenting out omp barrier in last line of print_result funtion
 * */
